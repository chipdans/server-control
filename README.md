...
## Диагностика Minecraft (beta.19)

На экране состояния появились вкладки «Minecraft» и «Домашний сервер».
Первая показывает реальные TPS, среднее время тика (MSPT), CPU и память Java,
а также пояснение: нормальная скорость, небольшой запас или замедление.
Метрики компьютера сохранены на второй вкладке; небольшое окно можно прокручивать.

TPS/MSPT запрашиваются локально через уже настроенный RCON раз в 30 секунд
командой `forge tps` или `neoforge tps`. Используется только итог по всему серверу.
При отключённом RCON, неподдерживаемой команде или тайм-ауте причина видна в окне;
приложение не меняет настройки RCON, не открывает порты и не оценивает TPS по CPU.
CPU и память доступны независимо от RCON. CPU Java — доля мощности всех логических
ядер; память — резидентная память процесса, а не размер кучи или установленный лимит.

Java определяется внутри службы активной сборки, включая запуск через tmux.
Короткий тайм-аут диагностики не меняет состояние запущенности сервера.
Кэш привязан к запуску и процессу; после перезапуска, смены сборки или потери связи
старые значения не выдаются за текущие. Несколько клиентов делят частоту запросов.
Пароль RCON остаётся на сервере и не передаётся в снимке состояния или кэше.
Эти показатели помогают заметить задержки; конкретный мод нужно искать профилировщиком.

## Исправление состояния Minecraft (beta.18)

Готовность подтверждается ответом Minecraft или сообщением `Done` в журнале текущего запуска. Результат сохраняется до смены процесса, сборки или журнала; краткий сбой status ping больше не возвращает сервер в состояние запуска. Большие журналы читаются частями, после подтверждения готовности анализ старых логов прекращается. Учитывается `server-ip`; недоступное число игроков отображается без `None`.
