# Обновление существующей установки 0.3.x до 1.0.6
...
cd D:\Code\server
git fetch origin
git tag -a v1.0.6 origin/main -m "Server Control 1.0.6"
...
```bash
cd /tmp
curl -fL https://github.com/chipdans/server-control/releases/download/v1.0.6/ServerControl-Agent.zip -o ServerControl-Agent.zip
curl -fL https://github.com/chipdans/server-control/releases/download/v1.0.6/ServerControl-Agent.zip.sha256 -o ServerControl-Agent.zip.sha256
...
Можно запустить текущий `ServerControl.exe`: он найдёт `v1.0.6`, проверит
...
2. Скачайте `ServerControl-Setup.zip` из release `v1.0.6`.
...
1. В заголовке клиента указано `Server Control 1.0.6`.
