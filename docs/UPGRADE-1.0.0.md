# Обновление существующей установки 0.3.x до 1.0.0
...
cd D:\Code\server
git fetch origin
git tag -a v1.0.0 origin/main -m "Server Control 1.0.0"
...
```bash
cd /tmp
curl -fL https://github.com/chipdans/server-control/releases/download/v1.0.0/ServerControl-Agent.zip -o ServerControl-Agent.zip
curl -fL https://github.com/chipdans/server-control/releases/download/v1.0.0/ServerControl-Agent.zip.sha256 -o ServerControl-Agent.zip.sha256
...
убедитесь, что показаны Agent `2.0.0`, protocol `2`.

### D. Обновить Windows client

Можно запустить текущий `ServerControl.exe`: он найдёт `v1.0.0`, проверит
...
2. Скачайте `ServerControl-Setup.zip` из release `v1.0.0`.
